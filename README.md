...
系统会自动把所有章节拆成独立的博客文章，并生成一个连载目录。支持的章节标题包括：
...
TXT 文件名会作为连载书名。例如上传 `我的小说.txt`，网站会生成“我的小说”连载目录，各章节按顺序排列。

再次上传同名 TXT 时，会更新对应章节。整篇文章没有多个章节标题时，仍然会作为普通单篇发布。